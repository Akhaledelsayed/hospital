...
// If access granted, get devices
$devices = get_all_devices($conn, $hospital_code);
?>
...
.table-wrapper::-webkit-scrollbar-thumb:hover {
    background: #555;
            ...
            .main-table td:nth-of-type(15)::before { content: "BMD_Code"; }
            ...
            .main-table td:nth-of-type(25)::before { content: "Device Safety Test"; }
            .main-table td:nth-of-type(26)::before { content: "Action"; }
        }
    </style>
</head>
<body>
    ...
    <div class="body">
        ...
        <section class="section-1">
            <h4 class="title">Manage Devices <a href="add_device.php">Add Device</a></h4>
            ...
            <?php if ($devices != 0) { ?>
                <div class="main-container" style="max-width: 95vw; overflow-x: auto;"></div>
                    ...
                    <tbody>
                        <?php $i = 0; foreach ($devices as $device) { ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($device['floor'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['Department'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['Department_now'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['Room'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['Accessories'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['Manufacturer'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['Origin'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['Company'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['Model'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['QT'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['BMD_Code'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['Arrival_Date'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['Installation_Date'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['purchaseorder_date'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['Price'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['Warranty_Period'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['warranty_start'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['warranty_end'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['company_contact'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['company_Tel'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['device_safety_test'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['hospital_code'] ?? '') ?></td>
                                <td><?= htmlspecialchars($device['assigned_user'] ?? '') ?></td>
                                <td>
                                    <?php if ($_SESSION['role'] === 'admin'): ?>
                                        <a href="edit-device.php?serial_number=<?= $device['serial_number'] ?>" class="btn edit-btn" title="Edit">
                                            <i class="fa fa-pencil"></i> Edit
                                        </a>
                                        <a href="delete-device.php?serial_number=<?= $device['serial_number'] ?>" class="btn delete-btn" title="Delete" onclick="return confirm('Are you sure you want to delete this device?');">
