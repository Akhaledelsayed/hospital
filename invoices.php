...
if (!$hasAccess) {
    ...
    $invoices= get_all_invoices($conn,$hospital_code);
...
.table-wrapper::-webkit-scrollbar-thumb:hover {
    background: #555;
            ...
            .main-table td:nth-of-type(11)::before { content: "Action"; }
        }
    </style>
</head>
<body>
    ...
    <div class="body">
        ...
        <section class="section-1">
            <h4 class="title">Manage Invoice <a href="add-invoices.php">Add Invoice</a></h4>
                    ...
                    <tbody>
                        <?php $i = 0; foreach ($invoices as $invoice) { ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($invoice['device_serial'] ?? '') ?></td>
                                <td><?= htmlspecialchars($invoice['device_name'] ?? '') ?></td>
                                 <td><?= htmlspecialchars($invoice['qt'] ?? '') ?></td>
                                 <td><?= htmlspecialchars($invoice['price'] ?? '') ?></td>

                                <td>
                                    <?php if ($_SESSION['role'] === 'admin'): ?>
                                    <a href="edit-invoices.php?id=<?= $invoice['id'] ?>" class="btn edit-btn" title="Edit">
                                        <i class="fa fa-pencil"></i> Edit
                                    </a>
                                    <a href="delete-invoices.php?id=<?= $invoice['id'] ?>" class="btn delete-btn" title="Delete" onclick="return confirm('Are you sure you want to delete this invoice?');">
