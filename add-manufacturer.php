...
<body>
	...
	<div class="body">
		...
		<section class="section-1">
			<h4 class="title">Add Manufacturer <a href="manufacturer.php">Manufacturer</a></h4>
			...
			<?php if (isset($_GET['error'])) { ?>
      	  	...
      	  	<?php } ?>
<form class="form-1" method="POST" action="app/add-manufacturer.php">
<?php
$country_codes = [
		"+20" => "🇪🇬 ",
		"+966" => "🇸🇦  ",
...
foreach ($fields as $name => $label) {
    ...
    if (in_array($name, [ "contact_mobile"])) {
        $type = "tel";
        $extra = 'pattern="[0-9]{6,}" title="Enter at least 6 digits"';
        echo '<div class="input-holder">
                ...
                <div style="display:flex; gap:8px;">
                    <select name="'.$name.'_code" class="input-1" style="max-width: 110px;" '.($required ? 'required' : '').'>';
						...
						echo '</select>
                    <input type="'.$type.'" name="'.$name.'" id="'.$name.'" class="input-1" style="flex:1;" '.$required.' '.$extra.' />
                </div>
              </div>';
    } else {
        echo '<div class="input-holder">
...
?>
    <button class="edit-btn" type="submit">Add Manufacturer</button>
</form>
