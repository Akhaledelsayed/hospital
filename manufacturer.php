...
if (!$hasAccess) {
    ...
    $manufacturers= get_all_factories($conn, $hospital_code);
...
.table-wrapper::-webkit-scrollbar-thumb:hover {
    background: #555;
            ...
            .main-table td:nth-of-type(3)::before { content: "Contact Name"; }
            .main-table td:nth-of-type(4)::before { content: "Contact Title"; }
            .main-table td:nth-of-type(5)::before { content: "Contact Mobile"; }
            .main-table td:nth-of-type(6)::before { content: "Contact Email"; }
            .main-table td:nth-of-type(7)::before { content: "Note"; }
            .main-table td:nth-of-type(8)::before { content: "Hospital-Code"; }
            .main-table td:nth-of-type(9)::before { content: "created by"; }
        }
    </style>
</head>
<body>
    ...
    <div class="body">
        ...
        <section class="section-1">
            <h4 class="title">Manage Manufacturer <a href="add-manufacturer.php">Add Manufacturer</a></h4>
                    ...
                    <tbody>
                        <?php $i = 0; foreach ($manufacturers as $manufacturer) { ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($manufacturer['manufacturer_name']) ?></td>
                                <td><?= htmlspecialchars($manufacturer['contact_name'] ?? '') ?></td>
                                <td><?= htmlspecialchars($manufacturer['contact_title'] ?? '') ?></td>
                                <td><?= htmlspecialchars($manufacturer['contact_mobile'] ?? '') ?></td>
                                <td><?= htmlspecialchars($manufacturer['contact_email'] ?? '') ?></td>
                                <td><?= htmlspecialchars($manufacturer['note'] ?? '') ?></td>
                                <td><?= htmlspecialchars($manufacturer['hospital_code'] ?? '') ?></td>
                                <td><?= htmlspecialchars($manufacturer['created_by'] ?? '') ?></td>
                                <td>
                                    <?php if ($_SESSION['role'] === 'admin'): ?>
  <a href="edit-manufacturer.php?manufacturer_name=<?= $manufacturer['manufacturer_name'] ?>" class="btn edit-btn"><i class="fa fa-pencil"></i> Edit</a>
  <a href="delete-manufacturer.php?manufacturer_name=<?= $manufacturer['manufacturer_name'] ?>" class="btn delete-btn" onclick="return confirm('Are you sure?')"><i class="fa fa-trash"></i> Delete</a>
  <?php else: ?>
